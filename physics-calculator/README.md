...
See the root [SECURITY.md](../SECURITY.md) and [REPO_HANDOFF.md](../REPO_HANDOFF.md) for the repository-wide policy and new-session restart instructions (including the strict rule that existing projects like this one are never deleted or altered on future additions — new projects are appended only, with root README updated by addition only).
